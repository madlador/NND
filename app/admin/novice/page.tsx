...
import { news, newsCategoryLabels, type NewsItem } from "@/lib/data/news"
import { Search, Plus, Pencil, Trash2 } from "lucide-react"
import { useToast } from "@/components/admin/admin-toast"

export default function AdminNovice() {
  ...
  const handleDelete = (id: string) => {
    ...
    setDeleteModal(null)
    showToast("Novica je bila uspešno izbrisana.")
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-2xl font-bold text-foreground">Novice</h1>
          <p className="text-sm text-muted-foreground mt-1">Upravljajte z novicami</p>
        </div>
        <Link
          href="/admin/novice/new"
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-left text-xs font-medium text-muted-foreground px-4 py-3">Izpostavljeno</th>
            ...
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id} className="border-b border-border last:border-b-0 hover:bg-secondary/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs text-muted-foreground">{newsCategoryLabels[item.category]}</span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    {item.isFeatured ? (
                      <span className="inline-flex px-2 py-0.5 text-xs font-medium rounded-full bg-amber-light text-amber">Da</span>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Link
                        href={`/admin/novice/${item.id}/edit`}
        ...
        <div className="fixed inset-0 bg-foreground/30 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl border border-border p-6 max-w-sm w-full" style={{ boxShadow: "var(--shadow-card-hover)" }}>
            ...
            <p className="text-sm text-muted-foreground mb-6">
              Ali ste prepričani, da želite izbrisati to novico? Dejanja ni mogoče razveljaviti.
            </p>
